...
public class BlogInsightSpecification {

    public static Specification<BlogInsight> hasAllCategories(List<Long> categoryIds) {
        return (root, query, cb) -> {
            // Avoid duplicates in result
            query.distinct(true);

            // Join to categories
            Join<BlogInsight, Category> categoryJoin = root.join("categories");
    ...
    public static Specification<BlogInsight> hasAllTags(List<Long> tagIds) {
        return (root, query, cb) -> {
            // Avoid duplicates in result
            query.distinct(true);

            // Join to categories
            Join<BlogInsight, Tag> tagJoin = root.join("tags");
    ...
    public static Specification<BlogInsight> hasAllCategoriesAndTags(List<Long> categoryIds, List<Long> tagIds) {
        return (root, query, cb) -> {
            // Tránh trùng kết quả
            query.distinct(true);
